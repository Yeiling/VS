...
namespace MyDelegate
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region 委托---字符串操作练习
            string[] strArry = new string[] { "1q2w3D", "2A3s4X5v", "1R2d3y4K", "5B" };
            ...
            {
                //return target.Length - first.Length; //求长度最大值
                return first.Length - target.Length; //求长度最小值
            }, out string maxOrMinLen);

            int[] intArry = new int[] { 9, 2, 10, 22, 1, 5, 0, 7, 3, 1, 1, 22, 5, 22, -4 };
            var result2 = GetMaxOrMin(intArry, (first, target) =>
            {
                //return target - first; //求最大值
                return first - target; //求最小值
            }, out int maxOrMin);
            #endregion
